...
object BridgeRepair2 {
  ...
  def applyOperators(factors: List[Long], ops: List[String]): Long = {
    factors.tail.zip(ops).foldLeft(factors.head) {
      case (result, (num, op)) => 
        op match {
          case "+" => result + num
          case "*" => result * num
          case "||" => (result.toString + num.toString).toLong
          case _ => throw new IllegalArgumentException("Operator can only be '+' or '*' or '||'")
        }
    }
  }

  def genNOperators(n: Int): List[List[String]] = {
    n match {
      case 0 => List(List())
      case _ => {
        val smaller = genNOperators(n-1)
        smaller.flatMap(comb => List("+" :: comb, "*" :: comb, "||" :: comb))
      }
    }
  }
