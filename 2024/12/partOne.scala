...
object GardenGroups {
  ...
  case class PlantMap(plants: Vector[String]) {
    ...
    def optionalCardinalNeighbors(x: Int, y: Int): List[Option[Char]] = 
      cardinalPositions(x, y).map{case (xx, yy) => get(xx, yy)}
  ...
  implicit class RegionSyntax(region: Region) {
    ...
    def perimeter: Int = {
      ...
      region.map{case (x, y) => regionMap.optionalCardinalNeighbors(x, y).count(_.forall(_ != '#'))}.sum
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val solution = plants.regions.map(r => r.area * r.perimeter).sum
